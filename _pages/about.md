---
permalink: /
title: "Dr. Thayabaran Kathiresan, Ph.D."
author_profile: true
redirect_from:
  - /about/
  - /about.html
---

<p class="lede">AI research &amp; innovation leader working across <em>speech &amp; language AI</em>, digital biomarkers and clinical AI.</p>

<ul class="roles">
  <li><span>Senior Director of Speech Analytics, <a href="https://redenlab.com/people/thayabaran-kathiresan-phd/">Redenlab</a></span><span class="fte">0.8 FTE</span></li>
  <li><span>Senior Postdoctoral Fellow, <a href="https://findanexpert.unimelb.edu.au/profile/1027702-thayabaran-kathiresan">University of Melbourne</a></span><span class="fte">0.2 FTE</span></li>
</ul>

<p class="statement">Leading the development and translation of artificial intelligence technologies for the clinically meaningful measurement of speech and communication.</p>

<div class="wave">
  <canvas id="wave" aria-hidden="true"></canvas>
  <div class="wave-legend">
    <span>Amplitude envelope</span>
    <span>Infant-directed speech &middot; 2.4 s</span>
  </div>
</div>

<script>
(function () {
  var canvas = document.getElementById("wave");
  if (!canvas || !canvas.getContext) return;
  var ctx = canvas.getContext("2d");

  var seed = 20260907;
  function rnd() { seed = (seed * 1664525 + 1013904223) % 4294967296; return seed / 4294967296; }

  var syllables = [], t = 0.06;
  while (t < 0.97) {
    var dur = 0.055 + rnd() * 0.075;
    syllables.push({ start: t, dur: dur, amp: 0.35 + rnd() * 0.65 });
    t += dur + 0.012 + rnd() * 0.045;
  }

  function envelopeAt(x) {
    var v = 0;
    for (var i = 0; i < syllables.length; i++) {
      var s = syllables[i], u = (x - s.start) / s.dur;
      if (u < 0 || u > 1) continue;
      var shape = u < 0.28 ? u / 0.28 : Math.pow(1 - (u - 0.28) / 0.72, 1.5);
      v = Math.max(v, s.amp * shape);
    }
    return v;
  }

  function draw() {
    var dpr = window.devicePixelRatio || 1;
    var w = canvas.clientWidth, h = canvas.clientHeight;
    if (!w) return;
    canvas.width = Math.round(w * dpr);
    canvas.height = Math.round(h * dpr);
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    ctx.clearRect(0, 0, w, h);

    var cs = getComputedStyle(document.documentElement);
    var accent = (cs.getPropertyValue("--accent") || "").trim() || "#0f6b63";
    var rule = (cs.getPropertyValue("--rule") || "").trim() || "#dbded9";
    var mid = h / 2;

    ctx.strokeStyle = rule; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(0, mid + 0.5); ctx.lineTo(w, mid + 0.5); ctx.stroke();

    ctx.strokeStyle = accent; ctx.globalAlpha = 0.55; ctx.lineWidth = 1.5;
    for (var px = 0; px <= w; px += 3) {
      var a = envelopeAt(px / w);
      if (a <= 0.002) continue;
      var jitter = 0.82 + 0.18 * ((px * 2654435761 % 97) / 97);
      var half = a * jitter * (h * 0.44);
      ctx.beginPath();
      ctx.moveTo(px + 0.5, mid - half);
      ctx.lineTo(px + 0.5, mid + half);
      ctx.stroke();
    }
    ctx.globalAlpha = 1;
  }

  draw();
  var timer;
  window.addEventListener("resize", function () { clearTimeout(timer); timer = setTimeout(draw, 120); });
  var toggle = document.querySelector(".theme-toggle, #theme-toggle");
  if (toggle) toggle.addEventListener("click", function () { setTimeout(draw, 60); });
  if (window.MutationObserver) {
    new MutationObserver(draw).observe(document.documentElement, { attributes: true, attributeFilter: ["data-theme"] });
  }
})();
</script>

## About

I am an AI research and innovation leader specialising in the development and translation of artificial intelligence technologies for speech, language and digital health.

As **Senior Director of Speech Analytics at Redenlab**, I lead research and innovation across speech AI, machine learning and digital biomarker development, with a particular focus on neurological and neurodevelopmental disorders. My work spans the full research lifecycle — from identifying clinically meaningful problems and developing new computational methods, through scientific validation, to translation into scalable clinical research technologies. I head the speech analytics function, contributing to the digital biomarker methods, speech data pipelines, remote monitoring workflows and endpoint analysis tools used in clinical trials that run across more than **300 sites in 25 countries**.

I lead multidisciplinary research programs involving speech foundation models, machine learning, automatic speech recognition, acoustic and phonetic modelling, natural language processing, digital biomarkers and generative AI. A major focus of my work is developing objective, AI-derived endpoints that can quantify clinically meaningful change in speech and communication.

My role also involves establishing research strategy, leading scientific collaborations, developing grant-funded research programs, mentoring scientists, and working with clinical, academic and industry partners to translate emerging AI research into robust and clinically meaningful capabilities.

Alongside my industry role, I am a **Senior Postdoctoral Fellow at the University of Melbourne**, Department of Audiology and Speech Pathology, where I contribute to research at the intersection of artificial intelligence, speech science and clinical research and supervise graduate students. Clinically, my work focuses on speech and language analysis in neurological populations, including ataxia, Friedreich's ataxia, ALS, multiple sclerosis and dementia.

## Research & Innovation

**Clinical AI & Digital Biomarkers**
Developing and validating AI-derived speech and language measures as objective digital biomarkers and clinical endpoints for neurological and neurodevelopmental disorders.

**Speech Foundation Models & Machine Learning**
Investigating self-supervised learning, transformer architectures and speech foundation models for extracting clinically meaningful representations from speech, including measures of articulation, intelligibility, naturalness, phonation, prosody and timing.

**AI-Based Speech & Phonetic Analytics**
Developing automated approaches for syllable segmentation, phoneme detection and alignment, pathological voice activity detection, speaker diarization, acoustic event detection and other components required for scalable clinical speech analysis.

**Natural Language & Generative AI**
Developing machine learning and NLP methods for the quantitative analysis of clinical language, including automated linguistic biomarkers, multilingual language analysis, large language models and emerging generative AI approaches.

**Responsible & Reproducible Clinical AI**
Establishing rigorous approaches to model validation, reproducibility, dataset and model provenance, performance monitoring and scientific governance, to support the responsible translation of AI models into clinical and academic research.

## Selected Research Programs

**Automated Research Audio Analysis**
Leading the development of an AI-driven research audio analysis and quality assurance framework incorporating automated task boundary detection, acoustic event detection, speaker counting and diarization, speaker-role identification, noise detection and pathological-speech voice activity detection.

**AI-Derived Clinical Endpoints**
Leading the development and validation of machine-learning-derived speech endpoints across multiple neurological disease programs, integrating acoustic, phonetic, prosodic and learned speech representations with clinically meaningful outcomes.

**Speech Intelligibility & Naturalness**
Developing machine learning approaches that transform perceptual assessments of speech intelligibility and naturalness into scalable, reproducible quantitative measures for clinical research.

**Phonetic & Temporal Speech Modelling**
Developing AI methods for syllable segmentation, phoneme detection and alignment, speech timing, and emerging measures of stress and articulatory behaviour in typical and pathological speech.

**Blood2Voice** *(exploratory research / work in progress)*
An exploratory research program investigating whether information embedded in speech can contribute to the prediction of molecular biomarkers associated with neurological disease, bridging digital phenotyping with molecular measures.

**Clinical ASR Validation**
Investigating the clinical validity of automatic speech recognition systems beyond conventional word error rate, including how transcription errors propagate into downstream linguistic biomarkers, diagnostic models and clinical AI outcomes.

## Research Leadership

My work increasingly focuses on building research programs and scientific capabilities at the intersection of AI, speech science and healthcare.

I lead and mentor scientists working across machine learning and speech AI, establish research priorities and validation strategies, and oversee the progression of new methods from exploratory research through scientific validation and clinical translation.

I also work closely with clinicians, academics, pharmaceutical researchers and technology teams to establish multidisciplinary research programs, develop grant-funded collaborations, and translate scientific advances into technologies that can be used in clinical research.

## Grants & Research Partnerships

I contribute to the development and leadership of collaborative research programs spanning academia, healthcare and industry. My work includes identifying new research opportunities, developing AI and digital biomarker methodologies for competitive grant applications, establishing multidisciplinary collaborations, and translating grant-funded research into reusable scientific capabilities.

**Funded research**

- **Forschungskredit UZH (Candoc) FK-19-069** (2018–2019) — *[How do infants identify their mothers by their voice](https://www.cl.uzh.ch/en/research-groups/phonetics/forschung/Voice/How-do-infants-identify-their-mothers-by-their-voice-The-role-of-temporal-characteristics-in-infant-directed-speech-style.html)*, as **project lead**. Automatic speaker recognition applied to test whether the expanded acoustic space of infant-directed speech makes individual mothers' voices more learnable.
- **MS Australia Incubator Grant 24-IGR2-0321** (2025) — *[Language function in multiple sclerosis](https://www.msaustralia.org.au/project/language-function-in-multiple-sclerosis/)*, as **Co-Investigator** with lead investigator Prof. Adam Vogel. Digital speech assessment, AI and sentiment analysis applied to detect early cognitive change in multiple sclerosis.
- **SNSF Project Funding** — *[The dynamics of indexical information in speech](https://data.snf.ch/grants/person/673199)* (PI Prof. Volker Dellwo, University of Zurich, 2019–2025), on which I was employed as a researcher.

- **NCCR Evolving Language** (SNSF National Centre of Competence in Research) — **external collaborator** on the Cooperation Project's *WP Accommodation* work package (PIs Burkart and Dellwo), contributing acoustic-variation methods and the dynamic auditory template matching model of vocal accommodation, tested on marmoset vocalisations. [(Year 1–3 report)](https://evolvinglanguage.ch/wp-content/uploads/NCCR-Report-Y3-.pdf)

I also contribute to research programs led by others and funded by the **NHMRC**, **MRFF** and **ARC**, as a collaborator rather than a named grant holder.

I am particularly interested in collaborations involving clinical AI, digital biomarkers, speech and language technologies, neurological and neurodevelopmental disorders, multimodal biomarkers, and AI-enabled clinical outcome measures.

## Career Path

Signal Processing & Engineering → Speech Science & Phonetics → Machine Learning & AI → Clinical Speech AI → Digital Biomarkers → AI Research & Innovation Leadership

I completed my **Ph.D. *summa cum laude*** in Computational Linguistics / Phonetics and Speech Sciences at the **University of Zurich** (2015–2019), funded by a Forschungskredit Candoc grant on which I was project lead. Prior to that I held an ERASMUS MUNDUS fellowship, earning M.Sc. degrees from **KTH Royal Institute of Technology** (Stockholm) and **Universitat Politècnica de Catalunya** (Barcelona), following a B.Eng. in Electronics and Communications Engineering from Anna University, Chennai.

My professional experience spans clinical-trial speech technology (Redenlab), ASR research (Telepathy Labs, Zurich), COVID-19 vocal biomarker diagnostics (VoiceMed, Luxembourg), forensic speaker verification (Centre for Forensic Phonetics and Acoustics), forensic voice casework as a consultant with [JP French International](https://www.jpfrench.com/dr-thayabaran-kathiresan/), and enterprise IT (Tata Consultancy Services).

I speak regularly on deepfake voice technology and its implications for security and society — most recently as an invited speaker at the Identity and Access Management Summits in Melbourne (2025) and Sydney (2026). I was an invited speaker at the University of Cambridge's [First Virtual Workshop on Automatic Diagnostics of COVID-19 Sounds](https://health-sounds.cl.cam.ac.uk/workshop20/) (2020), sponsored by the European Research Council, and an invited keynote speaker at the International Virtual Conference on Artificial Intelligence and Statistics (IVCAIS 2021), an invited expert in an international technology-transfer programme between the University of Zurich, Sweden and the National Institute of Informatics (NII), Japan, and a guest on Swiss national television (SRF *Einstein*) and radio explaining deepfake voice technology to a general audience.

## Expertise

**Leadership:** AI Research Strategy · Clinical AI · Digital Biomarkers · Speech & Language AI · Scientific Validation · Research Translation

**Technical:** Python · PyTorch · TensorFlow · Kaldi · speech foundation models & self-supervised learning · ASR and speaker recognition · acoustic and phonetic analysis · NLP and large language models · MLOps and cloud deployment (AWS)

---

**Profiles:** [Google Scholar](https://scholar.google.com/citations?user=bD7qkMkAAAAJ&hl=en) · [ORCID](https://orcid.org/0000-0002-7721-2699) · [University of Melbourne — Find an Expert](https://findanexpert.unimelb.edu.au/profile/1027702-thayabaran-kathiresan) · [Redenlab](https://redenlab.com/people/thayabaran-kathiresan-phd/)
